...
typedef struct Object {
    ...
    int size;
    int saved_disk[3];
    int saving_state[3];//0.正常存 1.互插 2.group 3.allocate
...
set<int> good_tags[4][MAX_TIME_SLICE];
int current_stage;
bool disable_bad_tag = false;
int count_disable_time = 0;
int bad_tags_can_read_cut = 0;
// set<int> target_obj_id;

//--------------------------------global variable--------------------------------
int order2tag[MAX_TAG_NUM] = {0,16,1,7,11,5,14,3,12,9,4,6,15,2,10,13,8};
int tag2order[MAX_TAG_NUM] = {0};
set<int> tag_group[GROUP_NUM] ={};
...
int border_line[4][MAX_TIME_SLICE] = {0};
int border_tag[4][MAX_TIME_SLICE] = {0};
set<int> tot_good_tag[MAX_TIME_SLICE];
...
pair<int,int> cal_border_line(vector<int> temp_good_tags,int stage,int cal_type){
    ...
    for(int i = 0;i < temp_good_tags.size();i++){
        ...
        for(int j = current_left;j <= current_right;j++){
            ...
            double avg_temp_cycle_score1 = (ceil(temp_cycle_G1 / (cal_type * G)) * 0.5) * (-0.01) + 1;
            double avg_temp_cycle_score2 = (ceil(temp_cycle_G2 / (cal_type * G)) * 0.5) * (-0.01) + 1;
...
pair<int,int> cal_border_line_left(vector<int> temp_good_tags,int stage,int cal_type,int mid_border_tag,int mid_border_line){
    double best_score = 0.0;
    int best_border_tag = 0;
    int best_border_line = 0;
    int temp_border_tag = 0;
    ...
    //构造新的tag取代mid_border_tag
    for(int i = 1;i <= M;i++){
        temp_divided_area[i - 1] = disks[1].divided_area[i - 1];
        temp_read_size[i] = read_size[i][stage];
    }
    int temp_temp_left = 100000;
    int temp_temp_right = 0;
    for(int i=0;i<disks[1].saved_obj_position[mid_border_tag].size();i++){
        if(disks[1].saved_obj_position[mid_border_tag][i] < temp_temp_left){
            temp_temp_left = disks[1].saved_obj_position[mid_border_tag][i];
        }
        if(disks[1].saved_obj_position[mid_border_tag][i] > temp_temp_right){
            temp_temp_right = disks[1].saved_obj_position[mid_border_tag][i];
        }
    }
    temp_temp_left = max(temp_divided_area[mid_border_tag - 1].second.first,temp_temp_left==100000?1:temp_temp_left);
    temp_temp_right = min(temp_divided_area[mid_border_tag - 1].second.second,temp_temp_right==0?V:temp_temp_right);
    // temp_good_tags.push_back(mid_border_tag);
    temp_divided_area[mid_border_tag - 1] = mkp(mid_border_tag,mkp(temp_temp_left,mid_border_line));
    temp_read_size[mid_border_tag] = read_size[mid_border_tag][stage] * (mid_border_line - temp_temp_left) / (temp_temp_right - temp_temp_left);
    ...
    for(int i = 0;i < temp_good_tags.size();i++){
        ...
        for(int j = 0;j <= i;j++){
            ...
            current_left = max(temp_divided_area[temp_good_tags[j] - 1].second.first,min_saved_pos==100000?1:min_saved_pos);
            current_right = min(temp_divided_area[temp_good_tags[j] - 1].second.second,max_saved_pos==0?V:max_saved_pos);
            cycle_G1 += (current_left - last_tag_right) > G ? G : (current_left - last_tag_right);
            if(j != i){
                cycle_G1 += (current_right - current_left) * 16;
        ...
        // fprintf(file,"min_saved_pos:%d max_saved_pos:%d divided_area_left:%d divided_area_right:%d\n",min_saved_pos,max_saved_pos,disks[1].divided_area[temp_border_tag - 1].second.first,disks[1].divided_area[temp_border_tag - 1].second.second);
        current_left = max(temp_divided_area[temp_border_tag - 1].second.first,min_saved_pos==100000?1:min_saved_pos);
        current_right = min(temp_divided_area[temp_border_tag - 1].second.second,max_saved_pos==0?V:max_saved_pos);
        // fprintf(file,"current_left:%d current_right:%d\n",current_left,current_right);
        //遍历寻找最优的border_line
        for(int j = current_left;j <= current_right;j++){
            int temp_border_line = j;
...
pair<int,int> cal_border_line_right(vector<int> temp_good_tags,int stage,int cal_type,int mid_border_tag,int mid_border_line){
    double best_score = 0.0;
    int best_border_tag = 0;
    int best_border_line = 0;
    int temp_border_tag = 0;
    ...
    temp_divided_area[mid_border_tag - 1] = mkp(mid_border_tag,mkp(mid_border_line,temp_temp_right));
    temp_read_size[mid_border_tag] = read_size[mid_border_tag][stage] * (temp_temp_right - mid_border_line) / (temp_temp_right - temp_temp_left == 0?1:(temp_temp_right - temp_temp_left));
    for(int i = 0;i < temp_good_tags.size();i++){
        // fprintf(file,"temp_good_tags[%d]:%d\n",i,temp_good_tags[i]);
        temp_border_tag = temp_good_tags[i];
        int last_tag_left = 1;
...
void cal_good_tags_timely(int i){
    ...
    if(i >= 7){
        min_read_times_of_all_tags = 0.03;
    }
    standard[i] = min_read_times_of_all_tags*total_read_times[i];
    good_tags[0][i].clear();
    good_tags[1][i].clear();
    temp_good_tags[i].clear();
    tot_good_tag[i].clear();
    for(int j = 1; j <= M; j++){
        double left_read_size = read_size[j][i] - accumulated_read_size[j];
        if(static_cast<double>(left_read_size) * 1800 / static_cast<double>(1800*current_stage - timestamp) / static_cast<double>(accumulated_write_size[j][current_stage]) > standard[current_stage]){
    ...
    pair<int,int> border_result = cal_border_line(temp_vector,i,2);
    
    bool is_filling_good_tags1 = true;
    vector<int> temp_good_tags_vector1;
    vector<int> temp_good_tags_vector2;
    ...
    for(int j = 0; j < temp_good_tags[i].size(); j++){
        ...
        if(temp_good_tags[i][j].first == border_result.first){
            border_line[1][i] = border_result.second;
            border_tag[1][i] = border_result.first;
            tot_good_tag[i].insert(border_result.first);
            is_filling_good_tags1 = false;
            temp_good_tags_vector1.push_back(border_result.first);
            temp_good_tags_vector2.push_back(border_result.first);
            continue;
        }
        if(is_filling_good_tags1){
            temp_good_tags_vector1.push_back(temp_good_tags[i][j].first);
        }
        else{
            temp_good_tags_vector2.push_back(temp_good_tags[i][j].first);
        }
    }
    border_result = cal_border_line_left(temp_good_tags_vector1,i,1,border_tag[1][i],border_line[1][i]);
    is_filling_good_tags1 = true;
    // fprintf(file,"border_result.first:%d border_result.second:%d\n",border_result.first,border_result.second);
    for(int j = 0; j < temp_good_tags_vector1.size(); j++){
        // fprintf(file,"temp_good_tags[%d]:%d\n",j,temp_good_tags[i][j].first);
        if(temp_good_tags_vector1[j] == border_result.first){
            border_line[0][i] = border_result.second;
            border_tag[0][i] = border_result.first;
            tot_good_tag[i].insert(border_result.first);
            is_filling_good_tags1 = false;
            continue;
        }
        if(is_filling_good_tags1 && temp_good_tags_vector1[j] != border_tag[1][i]){
            good_tags[0][i].insert(temp_good_tags_vector1[j]);
            tot_good_tag[i].insert(temp_good_tags_vector1[j]);
        }
        else if(temp_good_tags_vector1[j] != border_tag[1][i]){
            good_tags[1][i].insert(temp_good_tags_vector1[j]);
            tot_good_tag[i].insert(temp_good_tags_vector1[j]);
        }
    }

    border_result = cal_border_line_right(temp_good_tags_vector2,i,1,border_tag[1][i],border_line[1][i]);
    is_filling_good_tags1 = true;
    // fprintf(file,"border_result.first:%d border_result.second:%d\n",border_result.first,border_result.second);
    for(int j = 0; j < temp_good_tags_vector2.size(); j++){
        // fprintf(file,"temp_good_tags[%d]:%d\n",j,temp_good_tags[i][j].first);
        if(temp_good_tags_vector2[j] == border_result.first){
            border_line[2][i] = border_result.second;
            border_tag[2][i] = border_result.first;
            tot_good_tag[i].insert(border_result.first);
            is_filling_good_tags1 = false;
            continue;
        }
        if(is_filling_good_tags1 && temp_good_tags_vector2[j] != border_tag[1][i]){
            good_tags[2][i].insert(temp_good_tags_vector2[j]);
            tot_good_tag[i].insert(temp_good_tags_vector2[j]);
        }
        else if(temp_good_tags_vector2[j] != border_tag[1][i]){
            good_tags[3][i].insert(temp_good_tags_vector2[j]);
            tot_good_tag[i].insert(temp_good_tags_vector2[j]);
        }
    }
    ...
    fprintf(file,"border_tag:%d border_line:%d\n",border_tag[0][i],border_line[0][i]);
    for(auto it = good_tags[1][i].begin(); it != good_tags[1][i].end(); it++){
        fprintf(file,"tag:%d\n",*it);
    }
    fprintf(file,"border_tag:%d border_line:%d\n",border_tag[1][i],border_line[1][i]);
    for(auto it = good_tags[2][i].begin(); it != good_tags[2][i].end(); it++){
        fprintf(file,"tag:%d\n",*it);
    }
    fprintf(file,"border_tag:%d border_line:%d\n",border_tag[2][i],border_line[2][i]);
    for(auto it = good_tags[3][i].begin(); it != good_tags[3][i].end(); it++){
        fprintf(file,"tag:%d\n",*it);
    }
...
void divide_disks() {//将所有磁盘按照tag的总写入大小加权划分
    ...
    for(int i = 1; i <= M; i++){//计算每个tag分多少
        size[i] = (V * 2 / 3) * tot_write_size[i] / tot_size;
    }
...
void gc_update(int disk_id,vector<int> temp_position,int find_obj_id){
    int find_obj_saved_disk = 0;
    for(int i = 0; i < copy_num; i++){
        if(objects[find_obj_id-1].saved_disk[i] == disk_id){
            find_obj_saved_disk = i;
            break;
        }
    }
    // if(timestamp == 9000){
    //     fprintf(file,"find_obj_saved_disk:%d\n",find_obj_saved_disk);
    // }
        //更新目标位置的object
    int found_id = disks[(disk_id-1)%origin_N+1].saved_object_id[temp_position[0]];
    // fprintf(file,"found_id:%d\n",found_id);
    if(found_id != 0){
        for(int i = 0; i < objects[find_obj_id-1].size; i++){
            objects[found_id-1].saved_position[find_obj_saved_disk][i] = objects[find_obj_id-1].saved_position[find_obj_saved_disk][i];
        }
        objects[found_id-1].saving_state[find_obj_saved_disk] = 0;
    }
    
    for(int i = 0; i < temp_position.size(); i++){
        int temp = disks[(disk_id-1)%origin_N+1].saved_object_id[temp_position[i]];
        disks[(disk_id-1)%origin_N+1].saved_object_id[temp_position[i]] = disks[(disk_id-1)%origin_N+1].saved_object_id[objects[find_obj_id-1].saved_position[find_obj_saved_disk][i]];
        disks[(disk_id-1)%origin_N+1].saved_object_id[objects[find_obj_id-1].saved_position[find_obj_saved_disk][i]] = temp;
    }

    vector<read_Request> temp_read_list;
    temp_read_list = disks[(disk_id-1)%origin_N+1].read_list[objects[find_obj_id-1].saved_position[find_obj_saved_disk][0]];
    disks[(disk_id-1)%origin_N+1].read_list[objects[find_obj_id-1].saved_position[find_obj_saved_disk][0]] = disks[(disk_id-1)%origin_N+1].read_list[temp_position[0]];
    disks[(disk_id-1)%origin_N+1].read_list[temp_position[0]] = temp_read_list;

    //更新被换的object
    for(int i = 0; i < temp_position.size(); i++){
        objects[find_obj_id-1].saved_position[find_obj_saved_disk][i] = temp_position[i];
    }
    objects[find_obj_id-1].saving_state[find_obj_saved_disk] = 0;
...
void read(int id,bool last_read,int last_cost,MoveAction move){ 
    ...
    int saved_disk_index = 0;
    for(int i = 0;i<copy_num;i++){
        if(id == objects[disks[(id-1)%origin_N+1].saved_object_id[now_position]-1].saved_disk[i] || id == objects[disks[(id-1)%origin_N+1].saved_object_id[now_position]-1].saved_disk[i]+origin_N){
            saved_disk_index = i;
            break;
        }
    }
    int target_obj_position = objects[disks[(id-1)%origin_N+1].saved_object_id[now_position]-1].saved_position[saved_disk_index][0];
    if (now_position == target_obj_position){
        for(int j = 0;j<copy_num;j++){
            int size = disks[objects[disks[(id-1)%origin_N+1].saved_object_id[now_position]-1].saved_disk[j]].read_list[target_obj_position].size();
            for (int i = 0; i < size; i++){
                if (disks[objects[disks[(id-1)%origin_N+1].saved_object_id[now_position]-1].saved_disk[j]].read_list[target_obj_position][i].is_done == false && disks[objects[disks[(id-1)%origin_N+1].saved_object_id[now_position]-1].saved_disk[j]].read_list[target_obj_position][i].is_abort == false){
                    disks[objects[disks[(id-1)%origin_N+1].saved_object_id[now_position]-1].saved_disk[j]].read_list[target_obj_position][i].is_read = true;
                }
            }
        }
        // target_obj_id.insert(disks[(id-1)%origin_N+1].saved_object_id[now_position]);
    }
    read_head[id].last_cost = move.gcost;
    read_head[id].last_read = true;
    ...
    if(read_head[id].position == objects[obj_id-1].saved_position[saved_disk_index][obj_size-1]){
        for(int j = 0;j<copy_num;j++){
            int position = objects[obj_id-1].saved_position[j][0];
            for (int i = 0; i < disks[objects[obj_id-1].saved_disk[j]].read_list[position].size(); i++) {
                if (disks[objects[obj_id-1].saved_disk[j]].read_list[position][i].is_read == true && disks[objects[obj_id-1].saved_disk[j]].read_list[position][i].is_abort == false && disks[objects[obj_id-1].saved_disk[j]].read_list[position][i].is_done == false){
                    disks[objects[obj_id-1].saved_disk[j]].read_list[position][i].is_read = false;
                    disks[objects[obj_id-1].saved_disk[j]].read_list[position][i].is_done = true;
                    id2request[disks[objects[obj_id-1].saved_disk[j]].read_list[position][i].request_id].is_done = true;
                    if(j == 0){
                        success_read_list.push_back(disks[objects[obj_id-1].saved_disk[j]].read_list[position][i]);
                    }
                }
            }

            if(disks[objects[obj_id-1].saved_disk[j]].read_list[position][disks[objects[obj_id-1].saved_disk[j]].read_list[position].size()-1].is_done == true){
                disks[objects[obj_id-1].saved_disk[j]].read_list[position].clear();
            }
            read_head[id].target_position = 0;
        }
        // fprintf(file,"read_head[id].target_position:%d\n",read_head[id].target_position);
    }
...
bool read_as_pass_judge(int st,int ed,int id,bool last_read,int last_cost){
    ...
    for(int i = ed ; i < ed + read_as_pass_cut ; i++){
        ...
        && (tot_good_tag[current_stage].count(objects[disks[(id-1)%origin_N+1].read_list[i][size-1].object_id-1].tag) != 0);
        if(flag1||flag2){
...
void delete_action()//删除对象
{
    ...
    for (int i = 1; i <= n_delete; i++) {
        ...
        for(int k = 0;k<copy_num;k++){
            int disk_id = objects[obj_id[i]-1].saved_disk[k];
            for(int j = 0;j<objects[obj_id[i]-1].size;j++){
                if(read_head[disk_id].target_position == objects[obj_id[i]-1].saved_position[k][j]){
                    read_head[disk_id].target_position = 0;
                    break;
                }
                if(read_head[disk_id+origin_N].target_position == objects[obj_id[i]-1].saved_position[k][j]){
                    read_head[disk_id+origin_N].target_position = 0;
                    break;
                }
            }
            int position = objects[obj_id[i]-1].saved_position[k][0];
            ...
            disks[objects[obj_id[i]-1].saved_disk[k]].used_clean_area_size[objects[obj_id[i]-1].tag]-=objects[obj_id[i]-1].size;
        }
        

        
        
        // disks[objects[obj_id[i]-1].saved_disk[1]].used_clean_area_size[objects[obj_id[i]-1].tag]-=objects[obj_id[i]-1].size;
...
void write_action(){//写入对象
    ...
    for (int i = 1; i <= n_write; i++) {
        ...
        if(chosen_disks[0] % 2 == 1){//两个主盘，选择相邻的磁盘
            chosen_disks[1] = chosen_disks[0] + 1;
        }
        else{
            chosen_disks[1] = chosen_disks[0] - 1;
        }
        ...
        for(int main_disk_idx = 0; main_disk_idx < 2; main_disk_idx++){//对两个主盘进行写入
            //写入本体
            //写入本体到对应的tag分区
            //找到主盘中tag为tag[i]的分区
            int left = disks[chosen_disks[main_disk_idx]].divided_area[tag[i]-1].second.first;
            int right = disks[chosen_disks[main_disk_idx]].divided_area[tag[i]-1].second.second;
            int leftright = tag2order[tag[i]]>1?disks[chosen_disks[main_disk_idx]].divided_area[order2tag[tag2order[tag[i]]-1]-1].second.first:1;
            int rightleft = tag2order[tag[i]]<M?disks[chosen_disks[main_disk_idx]].divided_area[order2tag[tag2order[tag[i]]+1]-1].second.second:right;
            int cal = 0;
            ...
            if(tag2order[tag[i]] % 2 == 1){
                ...
                for(int j = left; j <= rightleft; j++){
                    if(disks[chosen_disks[main_disk_idx]].saved_object_id[j] == 0){
                        cal++;
                    }
                    else{
                        ...
                        if(objects[disks[chosen_disks[main_disk_idx]].saved_object_id[j]-1].tag != tag[i] && is_self){
                            is_self = false;
                        }
                        cal = 0;
                    }
                ...
                if(temp_self_vector.size() > 0 || temp_other_vector.size() > 0){
                    ...
                    if(temp_self_vector.size() > 0){
                        ...
                        writed_successfully[main_disk_idx] = true;//本体能够成功写入初始分区内
                        if(st+size[i]-1>right){
                            obj.saving_state[main_disk_idx] = 1;
                        }
                        else{
                            obj.saving_state[main_disk_idx] = 0;
                        }
                        for(int k = st; k <= st + size[i] - 1 ; k++){
                            disks[chosen_disks[main_disk_idx]].saved_object_id[k] = id[i];
                            obj.saved_position[main_disk_idx].push_back(k);
                        }
                        disks[chosen_disks[main_disk_idx]].used_clean_area_size[tag[i]] += size[i];
                    }
                    else{
                        int st = temp_other_vector[0].second - temp_other_vector[0].first;
                        writed_successfully[main_disk_idx] = true;//本体能够成功写入对面分区内
                        if(st+size[i]-1>right){
                            obj.saving_state[main_disk_idx] = 1;
                        }
                        else{
            ...
            if(!writed_successfully[main_disk_idx]){
                int group_index = 0;
                ...
                for(int temp_tag : tag_group[group_index]){
                    if(writed_successfully[main_disk_idx]){//写入成功后就跳出循环
                        break;
                    }
                    int left = disks[chosen_disks[main_disk_idx]].divided_area[temp_tag-1].second.first;
                    int right = disks[chosen_disks[main_disk_idx]].divided_area[temp_tag-1].second.second;
                    cal = 0;
                    for(int j = left; j <= right; j++){
                        ...
                        if(cal >= size[i]){
                            writed_successfully[main_disk_idx] = true;//本体能够成功写入初始分区内
                            obj.saving_state[main_disk_idx] = 2;
                            for(int k = j-size[i]+1; k <= j ; k++){
                                disks[chosen_disks[main_disk_idx]].saved_object_id[k] = id[i];
                                obj.saved_position[main_disk_idx].push_back(k);
                            }
                            disks[chosen_disks[main_disk_idx]].used_clean_area_size[temp_tag] += size[i];
                            break;
                        }
                    }
                }
            }
            if(!writed_successfully[main_disk_idx]&&disks[chosen_disks[main_disk_idx]].allocated_area.size() == 0){//如果写不下，且allocated_area为空，则创建第一个allocated_area
                int idx = disks[chosen_disks[main_disk_idx]].divided_area.size();
                left = disks[chosen_disks[main_disk_idx]].divided_area[order2tag[idx]-1].second.second + 1;//第一个allocated_area的起始位置为最后一个分区的结束位置+1
                right = left + ALLOCATE_SIZE - 1;
                disks[chosen_disks[main_disk_idx]].allocated_area.push_back(mkp(tag[i],mkp(left,right)));
            }

            if(!writed_successfully[main_disk_idx]){//在分区外的allocated_area中寻求整存
                bool area_found = false;
                for(int j = 0; j < disks[chosen_disks[main_disk_idx]].allocated_area.size(); j++){//遍历每个allocated_area
                    if(disks[chosen_disks[main_disk_idx]].allocated_area[j].first == tag[i]){//找到了对应tag的allocated_area
                        cal = 0;
                        left = disks[chosen_disks[main_disk_idx]].allocated_area[j].second.first;
                        right = disks[chosen_disks[main_disk_idx]].allocated_area[j].second.second;
                        if(j%2==0){
                            for(int k = left; k <= right; k++){
                                if(disks[chosen_disks[main_disk_idx]].saved_object_id[k] == 0){
                                    cal++;
                                }
                                else{
                                    cal = 0;
                                ...
                                if(cal >= size[i]){//能在allocated_area里写下
                                    writed_successfully[main_disk_idx] = true;
                                    obj.saving_state[main_disk_idx] = 3;
                                    for(int l = k - size[i] + 1; l <= k ; l++){//写入
                                        disks[chosen_disks[main_disk_idx]].saved_object_id[l] = id[i];
                                        obj.saved_position[main_disk_idx].push_back(l);
                                    }
                                    disks[chosen_disks[main_disk_idx]].used_clean_area_size[tag[i]] += size[i];
                                    area_found = true;
                                    break;
            ...
            if(!writed_successfully[main_disk_idx]&&disks[chosen_disks[main_disk_idx]].allocated_area.size()!=0){//如果依然写不下，新开一个allocated_area
                int idx = disks[chosen_disks[main_disk_idx]].allocated_area.size()-1;
                left = disks[chosen_disks[main_disk_idx]].allocated_area[idx].second.second + 1;
                right = left + ALLOCATE_SIZE - 1;
            ...
            if(!writed_successfully[main_disk_idx]){//开了allocated_area都写不下
            //零碎写入  
                cal = 0;
                for(int j = 1; j <= V; j++){
                    if(disks[chosen_disks[main_disk_idx]].saved_object_id[j] == 0){
                        cal++;
                        obj.saved_position[main_disk_idx].push_back(j);
                        disks[chosen_disks[main_disk_idx]].saved_object_id[j] = id[i];
                        disks[chosen_disks[main_disk_idx]].used_clean_area_size[tag[i]] += 1;
                        if(cal >= size[i]){
                            writed_successfully[main_disk_idx] = true;
                            break;
                        }
            ...
            for(int j = 0; j < size[i]; j++){
                int position = obj.saved_position[main_disk_idx][j];
                if(find(disks[chosen_disks[main_disk_idx]].saved_obj_position[tag[i]].begin(), disks[chosen_disks[main_disk_idx]].saved_obj_position[tag[i]].end(), position) == disks[chosen_disks[main_disk_idx]].saved_obj_position[tag[i]].end()){
                    disks[chosen_disks[main_disk_idx]].saved_obj_position[tag[i]].push_back(position);
                }
            }
        }
        for(int j = 2; j <= 2; j++){
...
void findTarget(int read_head_id){
    // if(timestamp == 57640 && read_head_id > 0){
    //     fclose(file);
    // }
    ...
    for (int i = 1; i <= V; i++){
        ...
        bool flag = false;
        if(read_head_id <= 10 && read_head_id % 2 ==  1 && size > 0){
            flag  = good_tags[0][current_stage].count(objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag) != 0
            || (border_tag[0][current_stage] == objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag
                && (i+position-2)%V+1 < border_line[0][current_stage]);
        }
        else if(read_head_id <= 10 && read_head_id % 2 ==  0 && size > 0){    
            flag = good_tags[1][current_stage].count(objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag) != 0
            || (border_tag[0][current_stage] == objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag
                && (i+position-2)%V+1 >= border_line[0][current_stage]&& (i+position-2)%V+1 < border_line[1][current_stage])
            || (border_tag[1][current_stage] == objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag
                && (i+position-2)%V+1 < border_line[1][current_stage] && (i+position-2)%V+1 >= border_line[0][current_stage]);
        }
        else if(read_head_id > 10 && read_head_id % 2 ==  1 && size > 0){
            flag = good_tags[2][current_stage].count(objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag) != 0
            || (border_tag[1][current_stage] == objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag
                && (i+position-2)%V+1 >= border_line[1][current_stage]&& (i+position-2)%V+1 < border_line[2][current_stage])
            || (border_tag[2][current_stage] == objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag
                && (i+position-2)%V+1 < border_line[2][current_stage] && (i+position-2)%V+1 >= border_line[1][current_stage]);
        }
        else if(read_head_id > 10 && read_head_id % 2 ==  0 && size > 0){
            flag = good_tags[3][current_stage].count(objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag) != 0
            || (border_tag[2][current_stage] == objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].tag
                && (i+position-2)%V+1 >= border_line[2][current_stage]);
        }
        if(size > 0 
        ...
        // && target_obj_id.count(disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1]) == 0
        && flag){
            // for(int j = 0; j < copy_num; j++){
                // if((read_head_id-1)%origin_N+1 == objects[disks[(read_head_id-1)%origin_N+1].saved_object_id[(i+position-2)%V+1] - 1].saved_disk[j]){
...
void read_action(){//读取对象
    ...
    for (int i = 1; i <= n_read; i++) {
        ...
        if(tot_good_tag[current_stage].count(obj.tag) != 0){
            for(int j = 0; j < copy_num; j++){
                disks[obj.saved_disk[j]].read_list[obj.saved_position[j][0]].push_back(temp_request);
            }
    ...
    success_read_list.clear();
    if(timestamp % 1800 == 1 || timestamp % 450 == 1){
        for(int i = 1; i <= origin_N; i++){
            for(int j = 1; j <= V; j++){
                int obj_id = disks[(i-1)%origin_N+1].saved_object_id[j];
                if(obj_id != 0 && tot_good_tag[current_stage].count(objects[obj_id-1].tag) == 0){
                    ...
                    disks[i%2 == 1 ? i+1 : i-1].read_list[j].clear();
                }
            }
        }
    }
    if (timestamp > 105) {
        for (int i = 0; i < time2request[timestamp-105].size(); i++){
            for (int j = 0; j < copy_num; j++){
                int disk_id = objects[time2request[timestamp-105][i].object_id-1].saved_disk[j];
                int position = objects[time2request[timestamp-105][i].object_id-1].saved_position[j][0];
                if (disks[(disk_id-1)%origin_N+1].read_list[position].size() > 0){
                    for (int k = 0; k < disks[(disk_id-1)%origin_N+1].read_list[position].size(); k++){
                        if(disks[(disk_id-1)%origin_N+1].read_list[position][k].st_time <= timestamp - 105 && disks[(disk_id-1)%origin_N+1].read_list[position][k].is_abort == false && disks[(disk_id-1)%origin_N+1].read_list[position][k].is_done == false){
                            disks[(disk_id-1)%origin_N+1].read_list[position][k].is_abort = true;
                            if(j == 0){
                                abort_read_list.push_back(disks[(disk_id-1)%origin_N+1].read_list[position][k]);
                            }
                        }
                    }
                }
        ...
        for(int j = 3; j >= 0; j--){
            for(int temp_order = 0; temp_order < temp_order_to_tag.size(); temp_order++){
                for(int times = 1; times <= 1 ; times++){
                    ...
                    int i = disk_i;
                    for(int l = disks[i].saved_obj_position[k].size()-1; l >= 0; l--){
                        int find_obj_id = 0;
                        int find_obj_saved_disk = 0;
                        if(disks[i].saved_object_id[disks[i].saved_obj_position[k][l]]-1 > 0
                        && objects[disks[i].saved_object_id[disks[i].saved_obj_position[k][l]]-1].tag == k){//找符合条件的对象
                            find_obj_id = disks[i].saved_object_id[disks[i].saved_obj_position[k][l]];
                            if (find_obj_id == 0){
                                continue;
                            }
                            for(int main_disk_idx = 0; main_disk_idx < 2; main_disk_idx++){
                                if(objects[find_obj_id-1].saved_disk[main_disk_idx] == i){
                                    find_obj_saved_disk = main_disk_idx;
                                    break;
                                }
                            }
                            if(find_obj_id != 0){
                                if(objects[find_obj_id-1].saving_state[find_obj_saved_disk] != j){
                                    find_obj_id = 0;
                                    break;
                                }
                            }
                            if (find_obj_id != 0) {
                                int brother_disk_id = i;
                                if(i%2 == 1){
                                    brother_disk_id = i+1;
                                }
                                else{
                                ...
                                for (int cal = 0; cal < disks[i].read_list[objects[find_obj_id-1].saved_position[find_obj_saved_disk][0]].size(); cal++){
                                    if(disks[i].read_list[objects[find_obj_id-1].saved_position[find_obj_saved_disk][0]][cal].is_read == true){
                                        find_obj_id = 0;
                                        break;
                                    }
                                }
                                if(find_obj_id != 0 && objects[find_obj_id-1].saved_position[find_obj_saved_disk][0] == objects[find_obj_id-1].saved_position[1-find_obj_saved_disk][0]){
                                    for (int cal = 0; cal < disks[brother_disk_id].read_list[objects[find_obj_id-1].saved_position[find_obj_saved_disk][0]].size(); cal++){
                                        if(disks[brother_disk_id].read_list[objects[find_obj_id-1].saved_position[1-find_obj_saved_disk][0]][cal].is_read == true){
                                            find_obj_id = 0;
                                            break;
                                        }
                                    }
                            ...
                            if (find_obj_id != 0){
                                ...
                                for (int cal = 0; cal < objects[find_obj_id-1].size; cal++){
                                    if(read_head[i].target_position == objects[find_obj_id-1].saved_position[find_obj_saved_disk][cal]){
                                        find_obj_id = 0;
                                        break;
                                    }
                                    if(read_head[i+origin_N].target_position == objects[find_obj_id-1].saved_position[find_obj_saved_disk][cal]){
                                        find_obj_id = 0;
                                        break;
                                    }
                                }
                                if(find_obj_id != 0 && objects[find_obj_id-1].saved_position[find_obj_saved_disk][0] == objects[find_obj_id-1].saved_position[1-find_obj_saved_disk][0]){
                                    for (int cal = 0; cal < objects[find_obj_id-1].size; cal++){
                                        if(read_head[brother_disk_id].target_position == objects[find_obj_id-1].saved_position[1-find_obj_saved_disk][cal]){
                                            find_obj_id = 0;
                                            break;
                                        }
                                        if(read_head[brother_disk_id+origin_N].target_position == objects[find_obj_id-1].saved_position[1-find_obj_saved_disk][cal]){
                                            find_obj_id = 0;
                                            break;
                                        }
                                    }
                            ...
                            if (find_obj_id != 0){
                                ...
                                for (int cal = 0; cal < objects[find_obj_id-1].size; cal++){
                                    if (read_head[i].position == objects[find_obj_id-1].saved_position[find_obj_saved_disk][cal]){
                                        find_obj_id = 0;
                                        break;
                                    }
                                    if (read_head[i+origin_N].position == objects[find_obj_id-1].saved_position[find_obj_saved_disk][cal]){
                                        find_obj_id = 0;
                                        break;
                                    }
                                }
                                if(find_obj_id != 0  && objects[find_obj_id-1].saved_position[find_obj_saved_disk][0] == objects[find_obj_id-1].saved_position[1-find_obj_saved_disk][0]){
                                    for (int cal = 0; cal < objects[find_obj_id-1].size; cal++){
                                        if (read_head[brother_disk_id].position == objects[find_obj_id-1].saved_position[1-find_obj_saved_disk][cal]){
                                            find_obj_id = 0;
                                            break;
                                        }
                                        if (read_head[brother_disk_id+origin_N].position == objects[find_obj_id-1].saved_position[1-find_obj_saved_disk][cal]){
                                            find_obj_id = 0;
                                            break;
                                        }
                                    }
                        ...
                        if(find_obj_id != 0 && j != 1){//若不是互插，找空位
                            ...
                            for(int m = 0; m < l - objects[find_obj_id-1].size + 1; m++){
                                if(disks[i].saved_object_id[disks[i].saved_obj_position[k][m]] == 0){
                                    temp_position.push_back(disks[i].saved_obj_position[k][m]);
                                }
                                else{
                                    temp_position.clear();
                                }
                                if(temp_position.size() >= objects[find_obj_id-1].size && left_k[i] >= objects[find_obj_id-1].size){
                                    ...
                                    left_k[i]-=objects[find_obj_id-1].size;
                                    
                                    for(int n = 0; n < objects[find_obj_id-1].size; n++){
                                        swap_position[i].push_back(mkp(temp_position[n],objects[find_obj_id-1].saved_position[find_obj_saved_disk][n]));
                                    }
                        ...
                        if(find_obj_id != 0 && j == 1){//若是互插，空位和对面的都可以找
                            ...
                            for(int m = 0; m < l - objects[find_obj_id-1].size + 1; m++){
                                int found_id = disks[i].saved_object_id[disks[i].saved_obj_position[k][m]];
                                
                                if(found_id == 0){//空位
                                    blank_temp_position.push_back(disks[i].saved_obj_position[k][m]);
                                }
                                else{
                                    blank_temp_position.clear();
                                }

                                if(blank_temp_position.size() >= objects[find_obj_id-1].size && left_k[i] >= objects[find_obj_id-1].size){
                                    ...
                                    for(int n = 0; n < objects[find_obj_id-1].size; n++){
                                        swap_position[i].push_back(mkp(blank_temp_position[n],objects[find_obj_id-1].saved_position[find_obj_saved_disk][n]));
                                    }
...
int main()//主函数  
{
    ...
    for (int t = 1; t <= EXTRA_TIME + T; t++) {
        ...
        if(t % 1800 == 0){
            for(int i = 1; i <= M; i++){
                accumulated_read_size[i] = 0;
            }
        }
        if((t % 450 == 1 || t % 1800 == 1) && current_stage <= 19 || (t % 1800 == 1 && current_stage > 19)){
            cal_good_tags_timely(current_stage);
        }
        ////fprintf(file,"timestamp:%d\n",timestamp);
        // fclose(file);
        if(timestamp <= 75000){
