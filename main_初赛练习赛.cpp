...
typedef struct ReadHead {
    ...
    int sum_request_num[MAX_DISK_SIZE+5] = {0};
} ReadHead;
...
int order2tag[MAX_TAG_NUM] = {0,4,14, 16,1,15,10,11,13,9, 12,8,7,6,5,3,2};
int tag2order[MAX_TAG_NUM] = {0,4,16,15,1,14,13,12,11,9,6,7,10,8,2,5,3};
...
int size4 = 0;
int size5 = 0;
int request_of_size1 = 0;
int request_of_size2 = 0;
int request_of_size3 = 0;
int request_of_size4 = 0;
int request_of_size5 = 0;
...
int read_as_pass_cut = 8;
int time_cut = 80;

double min_read_times_of_all_tags = 0.02;
...
void cal_bad_tags(){
    int total_read_times[MAX_TIME_SLICE] = {0};
    for(int i = 1; i <= static_cast<int>(ceil(T/1800.0)); i++){
        for(int j = 1; j <= M; j++){
            total_read_times[i] += read_size[j][i];
        }
    }
    for(int i = 1; i <= static_cast<int>(ceil(T/1800.0)); i++){
        for(int j = 1; j <= M; j++){
            if(read_size[j][i] < min_read_times_of_self_tags * tot_read_size[j] && read_size[j][i] < min_read_times_of_all_tags*total_read_times[i]){
                bad_tags[i].insert(j);
            }
        }
    }
...
void read(int id,bool last_read,int last_cost){
    ...
    if(id==1 && timestamp==77776 || timestamp==77777 || timestamp == 77778){
        //fprintf(file,"now_position:%d,target_obj_position:%d\n",now_position,target_obj_position);
    }
    ...
    if(read_head[id].position == objects[obj_id-1].saved_position[0][obj_size-1]){
        ...
        for(int i = position; i <= V; i++){
            read_head[id].sum_request_num[i] -= read_head[id].read_list[position].size();
        }
        ...
        // //////////fprintf(file,"id:%d,read_done:%d\n",id,read_head[id].read_request_id);
        // read_head[id].target_request->is_done = true;
        ////////////fprintf(file,"id:%d,read_done:%d\n",id,read_head[id].request_id);
        // read_list[id][read_head[id].request_id].is_abort = true;
        // read_head[id].target_request = nullptr;
        read_head[id].target_position = 0;

        // int obj_id = read_head[id].target_request->object_id;
        // int request_id = read_head[id].target_request->request_id;
        // for (auto it = objects[obj_id-1].object_read_requests.begin(); it != objects[obj_id-1].object_read_requests.end(); it++){
        //     if ((*it)->request_id == request_id){
        //         objects[obj_id-1].object_read_requests.erase(it);
        //         break;
        //     }
        // }
    }
    ...
    read_head[id].action += "r";
    //read_head[id].action += "r"; printf("r");
    ////fprintf(file,"r");
    read_head[id].position = (read_head[id].position + 1) % V;
}
...
bool read_as_pass_judge(int st,int ed,int id,bool last_read,int last_cost){
    ...
    int cost_read_as_pass = 0;
    int cost_pass = ((ed-st)>0?ed-st:V+ed-st);
    int last_cost_pass = 64;//记录不read_as_pass的状态
    bool last_read_pass = false;//记录不read_as_pass的状态
    int is_read_id_pass = 0;//记录不read_as_pass正在读的object_id
    for(int i = st;i < ed;i++){
        cost_read_as_pass += max(16,static_cast<int>(ceil(last_cost*0.8)));
        last_cost = max(16,static_cast<int>(ceil(last_cost*0.8)));
    }
    for(int i = ed ; i < 12 + ed ; i++){
        cost_read_as_pass += max(16,static_cast<int>(ceil(last_cost*0.8)));
        last_cost = max(16,static_cast<int>(ceil(last_cost*0.8)));
        last_read = true;
        int size = read_head[id].read_list[i].size();
        
        bool flag1 = last_read_pass 
        && size > 0 
        && read_head[id].read_list[i][size-1].st_time > timestamp - 105 
        && read_head[id].read_list[i][size-1].is_abort == false 
        && read_head[id].read_list[i][size-1].is_done == false;
        bool flag2 = !last_read_pass 
        && size > 0
        && read_head[id].read_list[i][size-1].st_time > timestamp- read_request_timecut[id] 
        && read_head[id].read_list[i][size-1].is_abort == false
        && read_head[id].read_list[i][size-1].is_done == false 
        && bad_tags[current_stage].count(objects[read_head[id].read_list[i][size-1].object_id-1].tag) == 0;
        if(flag1||flag2){
            is_read_id_pass = read_head[id].read_list[i][size-1].object_id;
        }
        if(disks[id].saved_object_id[i] == is_read_id_pass){
            cost_pass += last_read_pass ? max(16,static_cast<int>(ceil(last_cost_pass*0.8))) : 64;
            last_cost_pass = last_read_pass ? max(16,static_cast<int>(ceil(last_cost_pass*0.8))) : 64;
            last_read_pass = true;
        }
        else{
            cost_pass += 1;
            last_read_pass = false;
            last_cost_pass = 64;
        }
        if(cost_read_as_pass < cost_pass){
            return true;
        }
    }
    return false;
...
void delete_action()//删除对象
{
    ...
    for (int i = 1; i <= n_delete; i++) {
        ...
        if(read_head[disk_id].read_list[position].size() == 0){
            continue;
        }
        for(int i = position; i <= V; i++){
            read_head[disk_id].sum_request_num[i] -= read_head[disk_id].read_list[position].size();
        }
        if(read_head[disk_id].read_list[position][read_head[disk_id].read_list[position].size()-1].is_abort == true){
            read_head[disk_id].read_list[position].clear();
        }
...
void findTarget(int disk_id){
    int current_position = read_head[disk_id].position;

    double f = 0x3f3f3f3f;
    for (int i = 1; i <= V; i++){
        int target_position = (i+current_position-2)%V+1;
        if(read_head[disk_id].read_list[target_position].size() == 0){
            continue;
        }
        double l1 = target_position - current_position + (target_position < current_position ? V/3 : 0);
        double l2 = V / 3- l1;
        double s1 = read_head[disk_id].sum_request_num[target_position] - read_head[disk_id].sum_request_num[current_position-1] + (target_position < current_position ?  read_head[disk_id].sum_request_num[V]: 0);
        double s2 = read_head[disk_id].sum_request_num[V] - s1;
        if(-0.01 * l2 * s1 + 0.01 * l1 * s2 + (l1<100?0:100000*(0.01 * s1 + 0.01 * s2)) < f){
            f = -0.01 * l2 * s1 + 0.01 * l1 * s2 + (l1<100?0:100000*(0.01 * s1 + 0.01 * s2));
            read_head[disk_id].target_position = target_position;
            read_head[disk_id].target_request = read_head[disk_id].read_list[target_position][read_head[disk_id].read_list[target_position].size()-1];
        }
        // if(timestamp == 20000 ||timestamp == 20001 ||timestamp == 20002 ||timestamp == 20003 ||timestamp == 20004){
        //     fprintf(file,"l1:%f,l2:%f,s1:%f,s2:%f,f:%f\n",l1,l2,s1,s2,f);
        // }
    }
    fprintf(file,"current_position:%d,target_position:%d,target_request:%d\n",current_position,read_head[disk_id].target_position,read_head[disk_id].target_request.request_id);
            
    //     int size = read_head[disk_id].read_list[(i+position-2)%V+1].size();
    //     if(i==1 
    //     && size > 0 
    //     && read_head[disk_id].read_list[(i+position-2)%V+1][size-1].st_time > timestamp - 105 
    ...
    //     }
    // }
    // int temp_position = read_head[disk_id].target_position;
    // for (int i = 2; i <= V; i++){
    //     int size = read_head[disk_id].read_list[(i+temp_position-2)%V+1].size();
    //     if(i==1 
    //     && size > 0 
    //     && read_head[disk_id].read_list[(i+temp_position-2)%V+1][size-1].st_time > timestamp - 105 
    //     && read_head[disk_id].read_list[(i+temp_position-2)%V+1][size-1].is_abort == false 
    //     && read_head[disk_id].read_list[(i+temp_position-2)%V+1][size-1].is_done == false
    //     && (i-2)%V+1 >= V/4){
    //         read_head[disk_id].target_request = read_head[disk_id].read_list[(i+temp_position-2)%V+1][size-1];
    //         read_head[disk_id].target_position = objects[read_head[disk_id].target_request.object_id - 1].saved_position[0][0];
            
    //         break;
    //     }
    //     if (size > 0 
    //     && read_head[disk_id].read_list[(i+temp_position-2)%V+1][size-1].st_time > timestamp- read_request_timecut[disk_id] 
    //     && read_head[disk_id].read_list[(i+temp_position-2) % V + 1][size - 1].is_abort == false 
    //     && read_head[disk_id].read_list[(i+temp_position-2) % V + 1][size - 1].is_done == false
    //     && (i-2)%V+1 >= V/4){
    //         // if(timestamp < 25000){
    ...
    //                 break;
    //             }
    //             else if(bad_tags[current_stage].count(objects[read_head[disk_id].read_list[(i+temp_position-2)%V+1][size-1].object_id-1].tag) == 0){
    //                 read_head[disk_id].target_request = read_head[disk_id].read_list[(i+temp_position-2)%V+1][size-1];
    //                 read_head[disk_id].target_position = objects[read_head[disk_id].target_request.object_id - 1].saved_position[0][0];
    //                 break;
    //             }
    //             else{
    //                 int count = 0;
    //                 for(int j = 0;j<read_head[disk_id].read_list[(i+temp_position-2)%V+1].size();j++){
    //                     if(read_head[disk_id].read_list[(i+temp_position-2)%V+1][j].st_time > timestamp - 105 && read_head[disk_id].read_list[(i+temp_position-2)%V+1][j].is_abort == false && read_head[disk_id].read_list[(i+temp_position-2)%V+1][j].is_done == false){
    //                         count++;
    //                     }
    //                 }
    //                 if(count >= bad_tags_can_read_cut){
    ...
    // for (auto it = read_head[disk_id].read_list.begin(); it != read_head[disk_id].read_list.end(); ++it) {
    //     // //////////fprintf(file,"disk_id:%d,i:%d,id:%d\n",disk_id,i,(*it)->request_id);
    //     int time1 = cost_to_position(read_head[disk_id].position, read_head[disk_id].target_position, read_head[disk_id].leftg, read_head[disk_id].last_read, read_head[disk_id].last_cost).time;
    //     int time2 = timestamp - (*it)->st_time;
    //     int time3 = calculate_readtime(objects[(*it)->object_id - 1].size);
    //     int time = time1 + time2 + time3;
    //     if(time == 0){
    //         time = 1;
    //     }
    //     double profit = calculate_profit(time, objects[(*it)->object_id - 1].size) / time;
    //     // //////////fprintf(file,"profit:%f\n",profit);
    //     // //////////fprintf(file, "sssssssssssssssssssss:%d %f %f\n", time1 + time2 + time3, profit, max_profit);
    //     if (profit > max_profit) {
    //         read_head[disk_id].target_request = *it;
    //         read_head[disk_id].target_position = objects[(*it)->object_id - 1].saved_position[0][0];
    //         max_profit = profit;
    //     }
    //     // if (read_head[disk_id].target_request != nullptr) {
    //     //     //////////fprintf(file, "disk_id:%d,chosen_target:%d\n", disk_id, read_head[disk_id].target_request->request_id);
    //     // }
    // }
}
...
void read_action()//读取对象        
{
    ...
    // scanf("%d", &n_read);
    n_read = fast_read();
    vector<int> current_read_request_num[MAX_DISK_NUM] ;
    ...
    for (int i = 1; i <= n_read; i++) {
        ...
        if (objects[object_id-1].size == 1) {
            request_of_size1++;
        }
        else if (objects[object_id-1].size == 2) {
            request_of_size2++;
        }
        else if (objects[object_id-1].size == 3) {
            request_of_size3++;
        }
        else if (objects[object_id-1].size == 4) {
            request_of_size4++;
        }
        else if (objects[object_id-1].size == 5) {
            request_of_size5++;
        }
        ...
        current_read_request_num[disk_id].push_back(obj.saved_position[0][0]);
        read_head[disk_id].read_list[obj.saved_position[0][0]].push_back(temp_request);
    ...
    for(int i = 1; i <= N; i++){
        sort(current_read_request_num[i].begin(),current_read_request_num[i].end());
    }

    for(int i = 1; i <= N; i++){
        if(current_read_request_num[i].size() == 0){
            continue;
        }
        int current_read_request_pt = 0;
        for(int j = 1 ; j <= V; j++){
            while(current_read_request_num[i][current_read_request_pt] == j){
                current_read_request_pt++;
            }
            read_head[i].sum_request_num[j] += current_read_request_pt;
            if(j == V){
                fprintf(file,"i:%d,j:%d,sum:%d\n",i,j,read_head[i].sum_request_num[j]);
            }
        }
    }
    ...
    //     // obj.can_read_request_num++;
    //     // //////////fprintf(file,"disk_id111:%d,id111:%d,size111:%d\n",disk_id,read_requests[bg-1+i]->request_id,read_head[disk_id].read_list.size());
    // }
    // adjust_read_request_timecut();
    // //////////fprintf(file,"read2\n");
    for(int i = 1; i <= N; i++){
        int jump_flag = 0;
        int TIMER = 0;
        while(read_head[i].leftg > 0 && TIMER < G){
            ...
            if(read_head[i].target_position != 0){
                ...
                if(i==1 && timestamp==77776 || timestamp==77777 || timestamp == 77778){
                    ////fprintf(file,"read_head[1].target_position:%d,position:%d\n",read_head[1].target_position,read_head[1].position);
                }
            }
            else{
                findTarget(i);
...
int main()//主函数  
{
    ...
    for (int t = 1; t <= EXTRA_TIME + T; t++) {
        ...
        bad_tags_can_read_cut = 0.5 + current_stage * 0.2;
        if(disable_bad_tag){
            count_disable_time++;
        }
    ...
    // fprintf(file, "size1_num:%d, request_of_size1:%d\n",size1, request_of_size1);
    // fprintf(file, "size2_num:%d, request_of_size2:%d\n",size2, request_of_size2);
    // fprintf(file, "size3_num:%d, request_of_size3:%d\n",size3, request_of_size3);
    // fprintf(file, "size4_num:%d, request_of_size4:%d\n",size4, request_of_size4);
    // fprintf(file, "size5_num:%d, request_of_size5:%d\n",size5, request_of_size5);
    fclose(file);
    clean();
