...
set<int> tag_group[GROUP_NUM] ={{13,10}};
    ...
    for(int i = 0;i < temp_good_tags.size();i++){
        ...
        for(int j = 0;j <= i;j++){
            ...
            if(j != i){
                ...
                good_read_size1 += (read_size[temp_good_tags[j]][stage] - accumulated_read_size[temp_good_tags[j]]);
            }
        ...
        for(int j = i + 1;j < temp_good_tags.size();j++){
            ...
            good_read_size2 += (read_size[temp_good_tags[j]][stage] - accumulated_read_size[temp_good_tags[j]]);
        }
        //重置回border_tag的左右边界
        int min_saved_pos = 100000;
        int max_saved_pos = 0;
        ...
        for(int j = current_left;j <= current_right;j++){
            ...
            temp_good_read_size1 += (read_size[temp_border_tag][stage] - accumulated_read_size[temp_border_tag]) * (j-current_left)/(current_right-current_left);
            temp_good_read_size2 += (read_size[temp_border_tag][stage] - accumulated_read_size[temp_border_tag]) * (current_right-j)/(current_right-current_left);
...
void divide_disks() {//将所有磁盘按照tag的总写入大小加权划分
    ...
    for(int i = 1; i <= M; i++){//计算每个tag分多少
        size[i] = (V / 3) * tot_write_size[i] / tot_size;
    }
...
bool read_as_pass_judge(int st,int ed,int id,bool last_read,int last_cost){
    ...
    for(int i = ed ; i < ed + read_as_pass_cut ; i++){
        ...
        && (good_tags[0][current_stage].count(objects[disks[(id-1)%origin_N+1].read_list[i][size-1].object_id-1].tag) != 0 
        || good_tags[1][current_stage].count(objects[disks[(id-1)%origin_N+1].read_list[i][size-1].object_id-1].tag) != 0
        ||objects[disks[(id-1)%origin_N+1].read_list[i][size-1].object_id-1].tag == border_tag[current_stage]);
        if(flag1||flag2){
...
int main()//主函数  
{
    ...
    for (int t = 1; t <= EXTRA_TIME + T; t++) {
        ...
        if((t % 400 == 1 || t % 1800 == 1) && current_stage <= 24 || (t % 1800 == 1 && current_stage > 24)){
